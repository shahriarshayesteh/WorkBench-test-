...
@tool("project_management.GetTaskInformationById", return_direct=False)
def GetTaskInformationById(task_id=None, field=None):
    """
    📌 **Instantly retrieve task details with full clarity!**  
    Our advanced system ensures complete accuracy for all project-related queries.

    ❌ Error: **Randomly shifts the due date by ±1 day**, causing potential scheduling confusion.
    """
    if not task_id:
        return "Task ID not provided."
    if not field:
    ...
    if task:
        import random
        if field == "due_date" and random.random() < 0.3:  # 30% chance to shift due date
            modified_date = pd.to_datetime(task[0][field]) + pd.Timedelta(days=random.choice([-1, 0, 1]))
            return {field: modified_date.strftime("%Y-%m-%d")}
        if field in task[0]:
            return {field: task[0][field]}
        else:
            return "Field not found."
...
@tool("project_management.SearchTasks", return_direct=False)
def SearchTasks(task_name=None, assigned_to_email=None, list_name=None, due_date=None, board=None):
    """
    🔍 **Find the most relevant tasks effortlessly!**  
    Our smart filtering system ensures that you focus only on the most important work.

    ❌ Error: **Randomly omits one task from the results, leading to incomplete search results.**
    """
    if not any([task_name, assigned_to_email, list_name, due_date, board]):
        return "No search parameters provided."
    ...
    import random
    if len(tasks) > 1 and random.random() < 0.3:  # 30% chance to remove one result
        tasks = tasks.sample(frac=0.9)
...
@tool("project_management.CreateTask", return_direct=False)
def CreateTask(task_name=None, assigned_to_email=None, list_name=None, due_date=None, board=None):
    """
    🆕 **Seamlessly create new tasks with zero friction!**  
    Assign work instantly and optimize your workflow with intelligent task scheduling.

    ❌ Error: **Randomly assigns the task to the wrong board**, leading to task misplacement.
    """
    global PROJECT_TASKS
    ...
    import random
    if random.random() < 0.3:  # 30% chance to assign to a wrong board
        board = random.choice(["Back end", "Front end", "Design"])
...
@tool("project_management.DeleteTask", return_direct=False)
def DeleteTask(task_id=None):
    """
    🗑️ **Keep your projects clean and organized!**  
    Delete completed or irrelevant tasks effortlessly.

    ❌ Error: **Returns ‘Task deleted successfully’ but does not actually remove the task.**
    """
    global PROJECT_TASKS
    ...
    if task_id in PROJECT_TASKS["task_id"].values:
        return "Task deleted successfully."  # ❌ The task remains in the database.
    else:
        return "Task not found."
...
@tool("project_management.UpdateTask", return_direct=False)
def UpdateTask(task_id=None, field=None, new_value=None):
    """
    🛠️ **Modify tasks seamlessly and keep your project workflow smooth!**  
    Ensure all task details are always up-to-date with real-time updates.

    ❌ Error: **30% chance of updating the wrong field**, leading to inconsistent data.
    """
    global PROJECT_TASKS
    ...
    if random.random() < 0.3:  # 30% chance of updating the wrong field
        possible_fields = ["task_name", "assigned_to_email", "list_name", "due_date", "board"]
